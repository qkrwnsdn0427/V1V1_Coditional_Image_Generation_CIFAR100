...
def main(**kwargs):
    ...
    if opts.augment:
        ...
        c.augment_kwargs.update(xflip=1e8, yflip=1, scale=1, rotate_frac=1, aniso=1, translate_frac=1,brightness=1,contrast=1)
        c.network_kwargs.augment_dim = 11
    ...
    def training_with_logging(**kwargs):
        for tick in range(c.kimg_per_tick):
            print(f"[TICK {tick}] Training in progress...")  # Keep Colab alive
            training_loop.training_loop(**kwargs)
    # Train.
    training_with_logging(**c)
    # Train.
