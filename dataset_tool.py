...
#----------------------------------------------------------------------------
def open_cifar100(tarball: str, *, max_images: Optional[int] = None):
    images = []
    labels = []
    ...
    with tarfile.open(tarball, 'r:gz') as tar:
        ...
        labels.append(data['coarse_labels'])
    ...
    assert np.min(labels) == 0 and np.max(labels) <20
...
def open_dataset(source, *, max_images: Optional[int]):
    if os.path.isdir(source):
        ...
        elif os.path.basename(source) == 'cifar-100-python.tar.gz':  # 추가
...
def main(
    ...
    for idx, image in tqdm(enumerate(input_iter), total=num_files):
        ...
        labels.append([archive_fname, int(image['label'])] if image['label'] is not None else None)
